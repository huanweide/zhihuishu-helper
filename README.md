...
│   ├── run.js             #   逻辑单测 458 项（jsdom + vm 模块注入）
...
- [x] **v0.6.27 网络可诊断化：失败分层诊断（超时/断网/HTTP/HTML 分类）+ 23 条判别性测试，444 项逻辑单测全绿（累计 514 项）**
- [x] **v0.6.28 诊断贯通 + M8 通道健康检测：求解层 hint 断裂修复、观察池 8 项核销收口，449 项逻辑单测全绿（累计 519 项）**
- [x] **v0.6.29 M8 学习记录：答题来源分布 + 学习时长累计 + 习惯分统计，9 条判别性测试，458 项逻辑单测全绿（累计 528 项）**
- [ ] M7 真实站点端到端验证（需登录态）
- [x] M8 面板打磨：通道健康检测、答题记录页、习惯分统计（核心三块完成）

**测试门禁**：逻辑单测 458/458 + 截屏测试 70/70 = **528 项全绿**
...
| 版本 | 逻辑单测 | 截屏测试 | 合计 | 全绿 | 关键变更 |
|------|---------|---------|------|------|----------|
| v0.1.0 | 211 | - | 211 | ✓ | 首个可用版本 |
| v0.2.1 | 293 | - | 293 | ✓ | 闭环加固，修 3 bug |
| v0.6.26 | 421 | 70 | 491 | ✓ | 方案 D 题干锚定 + 打分制 |
| v0.6.27 | 444 | 70 | 514 | ✓ | 网络失败分层诊断（round-24） |
| **v0.6.28** | **449** | **70** | **519** | ✓ | 求解层诊断贯通 + 通道健康检测（round-25） |
| **v0.6.29** | **458** | **70** | **528** | ✓ | M8 学习记录 + 习惯分统计（round-25 续） |

> **测试方法学（判别性测试）**：每轮修复都配套「先红后绿」证据——先 `git stash` 回滚源码、确认旧代码上该测试**必红**，再恢复确认**必绿**。
> 本轮（round-24）新增 23 条第 32 组用例，实测 **Baseline 23 条全红 / Candidate 444 全绿**，无任何一条「新旧代码都绿」的假判据。这是「修好了没有」从自评改为机器判据的硬约束来源。

### 7.2 与参考项目的能力 Benchmark

| 能力 | 本项目 | ocsjs(3428★) | cxmooc-tools(2695★) | 说明 |
|------|:---:|:---:|:---:|------|
| 自动播放 F1 | ✓ | ✓ | ✓ | 基础能力 |
| 断点续播 F2 | ✓ | ✗ | ✗ | 需自己做 localStorage 持久化 |
| AI 答题·题库通道 | ✓ | ✓ | ✓ | 走 TikuAdapter 协议 |
| AI 答题·LLM 投票兜底 | ✓ | 部分 | ✗ | 多次生成取众数抗随机 |
| 油猴零部署 | ✓ | ✓ | ✗（扩展） | 用户随时开关 |
| **故障可诊断** | ✓（v0.6.27 起） | ✗ | ✗ | 超时/断网/HTTP/HTML 四类分类 + 解决方案提示 |
| **通道健康检测** | ✓（v0.6.28 新增） | ✗ | ✗ | 一键并发点检题库 + 模型两条通道，逐条给出原因与建议 |
| **学习记录 / 习惯分** | ✓（v0.6.29 新增） | ✗ | ✗ | 答题来源分布 + 学习时长累计 + 习惯分统计，面板可视化 |
| 抗改版结构兜底 | ✓ | 部分 | 部分 | 选择器失效时通用兜底扫描 |

### 7.3 能力矩阵（可视化）

```mermaid
graph TD
    A[智慧树助手 v0.6.29] --> B[F1 自动播放]
    A --> C[F2 断点续播]
    A --> D[F3 AI 自动答题]
    A --> E[工程化保障]
    ...
    C --> C1[localStorage 持久化]
    C --> C2[跨课 / 跨 iframe]
    C --> C3[重启自动恢复]

    D --> D1[题库通道·TikuAdapter]
    D --> D2[LLM 投票兜底]
    D --> D3[答案回填 + 选中自检]
    D --> D4[故障分层诊断<br/>超时/断网/HTTP/HTML]
    D --> D5[通道健康检测<br/>一键点检题库+模型]
    D --> D6[学习记录 / 习惯分<br/>来源分布+时长+习惯分]
...
> 能力图用 Mermaid 渲染（GitHub 原生支持）。若查看环境不支持，可对应 7.1 / 7.2 两张表获得同等信息。
