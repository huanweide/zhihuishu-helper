...
│   ├── run.js             #   逻辑单测 444 项（jsdom + vm 模块注入）
...
**测试门禁**：逻辑单测 444/444 + 截屏测试 70/70 = **514 项全绿**
...
| **v0.6.27** | **444** | **70** | **514** | ✓ | 网络失败分层诊断（本轮 round-24） |
...
| 油猴零部署 | ✓ | ✓ | ✗（扩展） | 用户随时开关 |
| **故障可诊断** | ✓（v0.6.27 新增） | ✗ | ✗ | 超时/断网/HTTP/HTML 四类分类 + 解决方案提示 |
